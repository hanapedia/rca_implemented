...
class TraceRCA:
    ...
    def trace_to_invo(input_path, output_path):
        # input_path = Path(input_path)
        # output_path = Path(output_path)
        train_ticket_invo_encoding_main(input_path, output_path)
    ...
    def tracerca(self, input_path, output_path, history_path, log_file, fisher_threshold=1.0, ad_threshold=1.0, min_support_rate=0.1, quiet=False, k=100):
        ...
        selecting_feature_main(input_file=input_path, output_file=sf_output, output_cache_file=sf_cache_output,  history=history_path, fisher_threshold=fisher_threshold)

        ad_output = Path(self.path_root + '/anomalies.pkl')
        invo_anomaly_detection_main(input_file=input_path, output_file=ad_output, useful_feature=sf_output, cache_file=sf_cache_output, main_threshold=ad_threshold)
