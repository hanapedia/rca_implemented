...
def latency_source_50(prom_url, start_time, end_time, faults_name):
    ...
    # ORIGINAL QUERY
    # 'histogram_quantile(0.50, sum(irate(istio_request_duration_milliseconds_bucket{reporter="source", destination_workload_namespace="sock-shop"}[1m])) by (destination_workload, source_workload, le))',
    ...
    # ORIGINAL QUERY
    # 'sum(irate(istio_tcp_sent_bytes_total{reporter="source"}[1m])) by (destination_workload, source_workload) / 1000',
    response = requests.get(prom_url,
...
#        print(svc)
    ...
    latency_df.to_csv('./data/' + filename)
    return latency_df
...
def latency_destination_50(prom_url, start_time, end_time, faults_name):
    ...
    # ORIGINAL QUERY
    # 'histogram_quantile(0.50, sum(irate(istio_request_duration_milliseconds_bucket{reporter="destination", destination_workload_namespace="sock-shop"}[1m])) by (destination_workload, source_workload, le))',
    response = requests.get(prom_url,
    ...
    # ORIGINAL QUERY
    # 'sum(irate(istio_tcp_sent_bytes_total{reporter="destination"}[1m])) by (destination_workload, source_workload) / 1000',
    response = requests.get(prom_url,
...
def ctn_network(prom_url, start_time, end_time, pod_name):
    # ORIGINAL QUERY
    # 'sum(rate(container_network_transmit_packets_total{namespace="sock-shop", pod_name="%s"}[1m])) / 1000 * sum(rate(container_network_transmit_packets_total{namespace="sock-shop", pod_name="%s"}[1m])) / 1000' % (pod_name, pod_name),
    response = requests.get(prom_url,
...
def ctn_memory(prom_url, start_time, end_time, pod_name):
    # ORIGINAL QUERY
    # 'sum(rate(container_memory_working_set_bytes{namespace="sock-shop", pod_name="%s"}[1m])) / 1000 ' % pod_name,
    response = requests.get(prom_url,
...
def node_network(prom_url, start_time, end_time, instance):
    # ORIGINAL QUERY
    # 'rate(node_network_transmit_packets{device="eth0", instance="%s"}[1m]) / 1000' % instance,
    response = requests.get(prom_url,
...
def node_cpu(prom_url, start_time, end_time, instance):
    # ORIGINAL QUERY
    # 'sum(rate(node_cpu{mode != "idle",  mode!= "iowait", mode!~"^(?:guest.*)$", instance="%s" }[1m])) / count(node_cpu{mode="system", instance="%s"})' % (instance, instance),
    response = requests.get(prom_url,
...
def node_memory(prom_url, start_time, end_time, instance):
    # ORIGINAL QUERY
    # '1 - sum(node_memory_MemAvailable{instance="%s"}) / sum(node_memory_MemTotal{instance="%s"})' % (instance, instance),
    response = requests.get(prom_url,
...
#        print(metric['source_workload'] , metric['destination_workload'] )
    ...
    # ORIGINAL QUERY
    # 'sum(istio_requests_total{destination_workload_namespace="sock-shop"}) by (source_workload, destination_workload)',
    response = requests.get(prom_url,
...
if __name__ == '__main__':
    ...
    prom_url_range = prom_url + '_range'
    ...
    latency_df_source = latency_source_50(prom_url_range, start_time, end_time, faults_name)
    latency_df_destination = latency_destination_50(prom_url_range, start_time, end_time, faults_name)
    latency_df = latency_df_destination.iloc[:, 1:].add(latency_df_source.iloc[:, 1:], fill_value=0)
    
    svc_metrics(prom_url_range, start_time, end_time, faults_name)
